...
app = FastAPI()
router=APIRouter()
@router.get("/camera1", response_class=HTMLResponse)
def camera_details():
    camera_name = "Canon Digital camera EOS R50"
    image_url = "https://i.moyo.ua/img/gallery/5373/2/1628542_middle.jpg"
    description = """Effortlessly create stunning content with the Canon EOS R50 Mirrorless Camera.
                    Featuring 4K video capabilities, a 24.2 Megapixel and paired with the RF-S18-45mm IS STM Lens.
                    It is the perfect choice for content creators & vloggers.
    ...
    # Створення рядка для зірочок
    full_stars = "★" * int(rating)  # Заповнені зірочки
    empty_stars = "☆" * (5 - int(rating))  # Порожні зірочки
